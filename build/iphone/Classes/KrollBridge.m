...
NSString *Client_Programs_App$ModuleRequireFormat = @"(function(exports){"
...
@implementation Client_Programs_AppObject
...
- (void)dealloc
{
  ...
  RELEASE_TO_NIL(_client_programs_app);
  OSSpinLockLock(&krollBridgeRegistryLock);
...
{
  // create Client_Programs_App global object
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];

  // Load the "Client_Programs_App" object into the global scope
  NSString *basePath = (url == nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
  _client_programs_app = [[Client_Programs_AppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

  TiContextRef jsContext = [kroll context];
  TiValueRef tiRef = [KrollObject toValue:kroll value:_client_programs_app];

  NSString *_client_programs_appNS = [NSString stringWithFormat:@"T%sanium", "it"];
  TiStringRef prop = TiStringCreateWithCFString((CFStringRef)_client_programs_appNS);
  ...
  if (preload != nil) {
    for (NSString *name in preload) {
      KrollObject *ti = (KrollObject *)[_client_programs_app valueForKey:name];
...
- (KrollWrapper *)loadCommonJSModule:(NSString *)code withSourceURL:(NSURL *)sourceURL
{
  ...
  NSString *js = [[NSString alloc] initWithFormat:Client_Programs_App$ModuleRequireFormat, dirname, filename, code];
...
- (KrollWrapper *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
  ...
  if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
    @throw [NSException exceptionWithName:@"org.client_programs_app.kroll"
