...
static NSString *const kClient_Programs_AppJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
- (NSString *)_client_programs_appInjection
{
  if (pageToken == nil) {
  ...
  [html appendString:kClient_Programs_AppJavascript];
...
+ (NSString *)content:(NSString *)content withInjection:(NSString *)injection
{
  ...
  // attempt to make well-formed HTML and inject in our Client_Programs_App bridge code
             ...
             baseURL:(NSURL *)baseURL
{
  ...
  content = [[self class] content:content withInjection:[self _client_programs_appInjection]];
            ...
            mimeType:(NSString *)mimeType
{
  ...
  [request setValue:[NSString stringWithFormat:@"%lu", (localId++)] forHTTPHeaderField:@"X-Client_Programs_App-Local-Id"];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
  ...
  if ([scheme hasPrefix:@"http"] || [scheme isEqualToString:@"ftp"]
    ...
    if ([scheme isEqualToString:@"file"] || [scheme isEqualToString:@"app"]) {
      [LocalProtocolHandler setContentInjection:[self _client_programs_appInjection]];
    }
    return YES;
  }
