...
// in simulator we redefine to format for Client_Programs_App Developer console
...
#define DEFINE_EXCEPTIONS                                                                                      \
  ...
  {                                                                                                            \
    NSString *exceptionName = [@"org.client_programs_app." stringByAppendingString:NSStringFromClass([self class])];  \
    TiExceptionThrowWithNameAndReason(exceptionName, reason, subreason, location);                             \
  }                                                                                                            \
                                                                                                               \
  +(void)throwException : (NSString *)reason subreason : (NSString *)subreason location : (NSString *)location \
  {                                                                                                            \
    NSString *exceptionName = @"org.client_programs_app";                                                             \
    TiExceptionThrowWithNameAndReason(exceptionName, reason, subreason, location);                             \
  }
...
enum {
  UIClient_Programs_AppNativeItemNone = -1,
  UIClient_Programs_AppNativeItemSpinner = -2,
  UIClient_Programs_AppNativeItemProgressBar = -3,

  UIClient_Programs_AppNativeItemSlider = -4,
  UIClient_Programs_AppNativeItemSwitch = -5,
  UIClient_Programs_AppNativeItemMultiButton = -6,
  UIClient_Programs_AppNativeItemSegmented = -7,

  UIClient_Programs_AppNativeItemTextView = -8,
  UIClient_Programs_AppNativeItemTextField = -9,
  UIClient_Programs_AppNativeItemSearchBar = -10,

  UIClient_Programs_AppNativeItemPicker = -11,
  UIClient_Programs_AppNativeItemDatePicker = -12,

  UIClient_Programs_AppNativeItemInfoLight = -13,
  UIClient_Programs_AppNativeItemInfoDark = -14,

  UIClient_Programs_AppNativeItemDisclosure = -15,

  UIClient_Programs_AppNativeItemContactAdd = -16
};
...
#endif
/**
 *	TiThreadPerformOnMainThread should replace all Client_Programs_App instances of
