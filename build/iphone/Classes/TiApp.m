...
- (NSString *)systemUserAgent
{
  ...
  NSString *kClient_Programs_AppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s", "Appc", "eler", "ator", "Tita", "nium"];
  return [NSString stringWithFormat:@"%@/%s (%@)", kClient_Programs_AppUserAgentPrefix, TI_VERSION_STR, currentDeviceInfo];
}
...
+ (NSDictionary *)tiAppProperties
{
  ...
  if (props == nil) {
    ...
    if (jsonData) {
      ...
      // If we have no data...
      // This should never happen on a Client_Programs_App app using the node.js CLI
